    ...
    var inter = {
        ...
        makePatternRenderer: function (pattern) { // ...
            if (pattern) {
                ...
                var tokenizedPattern = this.tokenizePattern(pattern);
                return function (values) {
                    return tokenizedPattern.map(function (token) {
        ...
        makeFileSizeRenderer: function (numDecimals, numberSystemId) {
            ...
            return function (size) {
                if (size < 1000) {
                    return that.renderNumber(size, undefined, numberSystemId, undefined, numDecimals) + ' bytes';
                } else if (size < 1000000) {
                    return that.renderNumber(size / 1024, undefined, numberSystemId, undefined, numDecimals) + ' KB';
                } else if (size < 1000000000) {
                    return that.renderNumber(size / 1048576, undefined, numberSystemId, undefined, numDecimals) + ' MB';
                } else if (size < 1000000000000) {
                    return that.renderNumber(size / 1073741824, undefined, numberSystemId, undefined, numDecimals) + ' GB';
                } else {
                    return that.renderNumber(size / 1099511627776, undefined, numberSystemId, undefined, numDecimals) + ' TB';
                }
            };
        },
         ...
         * @param {Number} numDecimals (optional) The number of decimals.
            ...
            if (!dateInstance || !dateInstance.getHours) {
                if (dateInstance._time) {
                    ...
                    dateInstance = {
                        ...
                        getTime: function () {
                            return icalJsDate.toUnixTime() * 1000;
                        },
                        getDay: function () {
        ...
        makeDateNormalizerSource: function (variableName) {
            return this.normalizeDate.toString().replace(/^[^\{]+\{|\s*return \w+;?\s*\}$/g, '').replace(/dateInstance/g, variableName);
        },

// makeDateFieldRenderer!?
        renderDateField: function (d, fieldName, calendarId) {
        ...
        _fieldRendererByFormatCharAndDetailLevel: {
            G: [ // Era designator
                function (d, calendarId) {
                    return inter.calendars[calendarId].eraNames.abbreviated[d.getFullYear() > 0 ? 1 : 0];
                }
            ],
            y: [
                function (d, calendarId) {
                    return '0000'.slice(String(d.getFullYear()).length) + d.getFullYear();
                }
            ],
            //Y: [], // Week of Year
            //u: [], // Extended year
            ...
            Q: [
                function (d) {
                    return 1 + Math.floor(d.getMonth() / 3);
                },
                function (d) {
                    return '0' + (1 + Math.floor(d.getMonth() / 3));
                },
                function (d, calendarId) {
                    return inter.calendars[calendarId].quarterNames.format.abbreviated[Math.floor(d.getMonth()) / 3];
                },
                function (d, calendarId) {
                    return inter.calendars[calendarId].quarterNames.format.wide[Math.floor(d.getMonth()) / 3];
                }
            ],
            q: [
                function (d) {
                ...
                function (d, calendarId) {
                    return inter.calendars[calendarId].quarterNames.standAlone.abbreviated[Math.floor(d.getMonth()) / 3];
                },
                function (d, calendarId) {
                    return inter.calendars[calendarId].quarterNames.standAlone.wide[Math.floor(d.getMonth()) / 3];
                }
            ],
            M: [
                function (d) {
                ...
                function (d, calendarId) {
                    return inter.calendars[calendarId].monthNames.format.abbreviated[d.getMonth()];
                },
                function (d, calendarId) {
                    return inter.calendars[calendarId].monthNames.format.wide[d.getMonth()];
                }
            ],
            L: [
                function (d) {
                ...
                function (d, calendarId) {
                    return inter.calendars[calendarId].monthNames.standAlone.abbreviated[d.getMonth()];
                },
                function (d, calendarId) {
                    return inter.calendars[calendarId].monthNames.standAlone.wide[d.getMonth()];
                }
            ],
            //w: [], // Week of year
            //W: [], // Week of month
            ...
            D: [ // Day of year
                function (d) {
                    return (1 + Math.floor((d.getTime() - new Date(d.getFullYear(), 0, 1).getTime()) / 86400000));
                }
            ],
            F: [ // Day of week in month
                function (d) {
                    return (1 + Math.floor((d.getDate() - 1) / 7));
                }
            ],
            //g: [], // Modified Julian day
            E: [
                function (d, calendarId) {
                    return inter.calendars[calendarId].dayNames.format.abbreviated[d.getDay()];
                },
                undefined,
                undefined,
                function (d, calendarId) {
                    return inter.calendars[calendarId].dayNames.format.wide[d.getDay()];
                }
            ],
            //e: [], // Local day of week
            ...
            z: [
                function (d, calendarId) {
                    return d.getTimezoneOffset() < 0 ?
                        renderDateFormat(new Date(1970, 0, 1, 0, -d.getTimezoneOffset()), this.timeZoneFormats.hour[0], calendarId) :
                        renderDateFormat(new Date(1970, 0, 1, 0, d.getTimezoneOffset()), this.timeZoneFormats.hour[1], calendarId);
                }
            ]
        ...
        makeDateFieldTokenRenderer: function (fieldToken, calendarId) {
            calendarId = calendarId || 'gregorian';
            var renderers = this._fieldRendererByFormatCharAndDetailLevel[fieldToken.charAt(0)];
            if (renderers) {
                var detailLevel = Math.min(fieldToken.length, renderers.length) - 1;
                while (detailLevel > 0 && typeof renderers[detailLevel] === 'undefined') {
                    detailLevel -= 1;
                }
                return renderer[detailLevel];
            }
        },
...
*/
        ...
        makeDateFormatRenderer: function (format, calendarId) {
            calendarId = calendarId || 'gregorian';
            var tokens = this.tokenizeDateFormat(format);
            var that = this;
            return function dateFormatRenderer(d) {
...
*/
                        return that.renderDateField(d, token.value, calendarId);
                    }
                }).join('');
            };
        },
        ...
        makeDateRenderer: function (formatId, calendarId) {
            var that = this;
            var dataFormatRenderer = this.makeDateFormatRenderer(this.getDateFormat(formatId, calendarId), calendarId);
            return function (d) {
                return dateFormatRenderer(that.normalizeDate(d));
            };
        },
        ...
        makeDateIntervalRenderer: function (formatId, datePartOnly, calendarId) {
            ...
            if (greatestDifferences) {
                ...
                if (datePartOnly && matchFormatId) {
                    ...
                    return that.getPatternRenderer(
                        that.calendars[calendarId].dateIntervalFallbackFormat,
        ...
        applySpacingToCurrencyString: function (currencyString, currencyFormat, numberSystemId) {
            // Helper vars
            var beforeCurrency = currencyFormat.currencySpacing.beforeCurrency,
                afterCurrency = currencyFormat.currencySpacing.afterCurrency;

            // http://www.unicode.org/reports/tr35/tr35-numbers.html#Currencies - see currencySpacing
            var preRenderedCurrency = this.renderNumber(42, currencyFormat['default'], numberSystemId, '¤');

            var currencyStartMatched = new RegExp('^' + beforeCurrency.currencyMatch.source).test(currencyString),
                currencyEndMatched = new RegExp(afterCurrency.currencyMatch.source + '$').test(currencyString);

            var beforeCurrencySurroundingMatched = new RegExp(beforeCurrency.surroundingMatch.source + '¤').test(preRenderedCurrency),
        ...
        renderCurrencyShort: function (value, currencyId, numberSystemId, numDecimals) {
            var currency;
            if (this.currencies) {
                // Allow using it without --currencies flag
