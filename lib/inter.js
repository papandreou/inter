...
inter = {
    ...
    makePercentageRenderer: function (numDecimals, numberSystem) {
        return new Function("num", "return " + this.makeNumberRendererSource("100*num", numDecimals, "", " " + this.numberSymbols[numberSystem || 'latn'].percentSign) + ";");
    },
    ...
    makeFileSizeRenderer: function (numDecimals, numberSystem) {
        numberSystem = numberSystem || 'latn';
    ...
    makeNumberRendererSource: function (sourceVariableNameOrExpression, numDecimals, prefix, suffix, numberSystem) {
        var numberSymbols = this.numberSymbols[numberSystem || 'latn'];
    ...
    makeDateRenderer: function (formatId, calendarId) {
        ...
        if (formatId === 'hv') {
            ...
            format = this.getDateFormat("shortTime", calendarId).replace(/[\.:]i\s*/, "");
        } else {
            format = this.getDateFormat(formatId, calendarId);
            if (!format) {
    ...
    makeDateIntervalRenderer: function (formatId, datePartOnly, calendarId) {
        ...
        if (!greatestDifferences) {
            var bestMatchingDateIntervalFormatId = this.getBestICUFormatId(formatId, this.dateIntervalFormats, calendarId);
        ...
        if (greatestDifferences) {
            ...
            if (datePartOnly && matchFormatId) {
                ...
                if (dateFormat) {
                    ...
                    throw new Error("inter.renderDateInterval: No usable date interval format found for " + formatId + " (" + calendarId + ")");
                }
            }
        }
    },
        ...
        if (icuFormat) {
            ...
            if (bestCandidateFormatId) {
                ...
                if (matchFormatId) {
                    ...
                    return dateFormat && timeFormat && this.renderPattern([timeFormat, dateFormat], this.calendars[calendarId].defaultDateTimePattern);
                } else {
...
['timeZones', 'countries', 'regions', 'locales', 'currencies', 'scripts'].forEach(function (pluralName) {
